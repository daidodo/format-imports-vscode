...
export class SimpleImportRunner implements ImportRunner {
  ...
  private getExcludeUnusedImports(
    ...
    const isRemoveUnusedImports = this.configurationProvider.getConfiguration()
    ...
    chain(sortedImports)
      ...
      .forEach(x => {
        ...
        if (previousRange.isLineIntersecting(currentRange)) {
          rangesToDelete[rangesToDelete.length - 1] = previousRange.union(
            currentRange
          );
          return;
        }
